...
import com.weebly.mattkx4.morefurnaces.blocks.GoldFurnace;
...
public class TileEntityGoldFurnace extends TileEntity implements
		ISidedInventory {
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				GoldFurnace.updateGoldFurnaceState(this.burnTime > 0,
	...
	public boolean canSmelt() {
		if (this.slots[0] == null) {
			...
			/*
			 * The following two lines of code grab the display name (the name
			 * seen In-Game) and compares it to the strings "_ore", "ore" and
			 * "Ore" if the input item contains any of these strings then it
			 * wont be smelted.
			 * 
			 * ** WARNING: Has not tested with mod items that are non-ore items
			 * that contain the name ore ** example: "Uncooked Smores". Nor has
			 * this been tested with mod items that are ores but ** do not
			 * contain the strings previously described. ** It is expected that
			 * in the two examples the item would not and would cook
			 * respectively
			 */

			cookingItemName = this.slots[0].getDisplayName();
			if (cookingItemName.contains("_ore")
					|| cookingItemName.contains("ore")
					|| cookingItemName.contains("Ore")
					|| cookingItemName.contains("dust")
					|| cookingItemName.contains("Dust"))
				return false;

			/*
			 * A possible change to the above code in regards to compatibility
			 * with forge's ore dictionary (net.minecraftforge.oredict) would be
			 * to grab the ore dictionary unlocalized name, (the name the
			 * register uses) and compare it to the string "ore".
			 * 
			 * eg. uniqueOres[] = getOres(); cookingItemName =
			 * this.slots[0].getOreNames; if(cookingItemName == uniqueOres &&
			 * cookingItemName.contains("ore"))return false;
			 */
