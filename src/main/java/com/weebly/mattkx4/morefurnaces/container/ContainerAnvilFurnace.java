...
public class ContainerAnvilFurnace extends Container {
	 ...
	 */
	public ContainerAnvilFurnace(InventoryPlayer inventory,
			TileEntityAnvilFurnace tileentity) {
		this.anvilFurnace = tileentity;

		// setting this from SlotFurnace to simply Slot fixes the slots
		// inability to accept items from a click
	...
	public void addCraftingToCrafters(ICrafting icrafting) {
		...
		icrafting.sendProgressBarUpdate(this, 2,
				this.anvilFurnace.currentItemBurnTime);
	}
		...
		for (int i = 0; i < this.crafters.size(); i++) {
			...
			if (this.lastCookTime != this.anvilFurnace.repairTime) {
				icrafting.sendProgressBarUpdate(this, 0,
						this.anvilFurnace.repairTime);
			}

			if (this.lastBurnTime != this.anvilFurnace.burnTime) {
				icrafting.sendProgressBarUpdate(this, 1,
						this.anvilFurnace.burnTime);
			}
