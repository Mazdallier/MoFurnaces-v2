package com.weebly.mattkx4.morefurnaces.entity;
...
public class EntityCobaltKatana extends EntityMob {
	...
	public EntityCobaltKatana(World world) {
		...
		// Little bit of background knowledge, the number for the task denotes
		// the priority of the task
		this.tasks.addTask(0, new EntityAISwimming(this));
		this.tasks.addTask(2, new EntityAIWander(this, 1.0D));
		this.tasks.addTask(1, new EntityAIAttackOnCollide(this,
				EntityMob.class, 1.0D, true));
		this.tasks.addTask(1, new EntityAIAttackOnCollide(this,
	...
	/**
	 * quick check to see if the item that will be right-clicked onto CK Mob has
	 * a cooking result
	 * 
	 * @param item
	 * @param slot
	 ...
	 * 
	 * @param branch
	 *            : Which piece of text should the player receive
	 * @param player
	 *            : The player entity
	 */
	public void chatTree(int branch, EntityPlayer player) {
		...
		case 1: {
			if (randomInt == 0) {
				...
				player.addChatMessage(new ChatComponentText("Hey there "
						+ player.getDisplayName() + ", good to meet yah!"));
			} else if (randomInt == 2) {
				player.addChatMessage(new ChatComponentText(
						"My name's Cobalt Katana! It's nice to meet you "
								+ player.getDisplayName() + "!"));
			}
			greeted = true;
			return;
		...
		case 2: {
			player.addChatMessage(new ChatComponentText("You have added "
					+ newAttempts
					+ " more attempts to my furnace. Total attempts left: "
					+ attempts + "."));
			return;
		}
		// notify the player that they have nothing in hand
		case 3: {
			player.addChatMessage(new ChatComponentText("I'm sorry "
					+ player.getDisplayName() + ", you have nothing to cook."));
			return;
		}
		...
		case 4: {
			...
			if (heldItem.stackSize == 1) {
				player.addChatMessage(new ChatComponentText(heldItem
						.getDisplayName()
						+ " > "
						+ cookingResult.getDisplayName()));
				return;
			}
			// if there is more than one item
			player.addChatMessage(new ChatComponentText(heldItem.stackSize
					+ " " + heldItem.getDisplayName() + " > "
					+ cookingResult.stackSize + " "
					+ cookingResult.getDisplayName()));
			return;
		}
		...
		case 5: {
			if (attempts == 0) {
				player.addChatMessage(new ChatComponentText(
						"You have no more attempts for cooking, "
								+ player.getDisplayName() + "."));
				return;
			} else {
				player.addChatMessage(new ChatComponentText(
		...
		case 6: {
			player.addChatMessage(new ChatComponentText("I'm sorry "
					+ player.getDisplayName() + ", I can't cook that for you."));
			return;
		}
		default: {
			return;
		...
		if (!this.worldObj.isRemote) {
			...
			if (angerLevel == 0) {
				...
				boolean flag = heldItem != null
						&& (heldItem.getItem() == Items.diamond || heldItem
				...
				// if there are attempts available and there is an item to smelt
					...
					player.inventory.setInventorySlotContents(thisSlot,
							result.copy());
					// remove one attempt
					attempts--;
					 ...
					 * 
					 * //test code for smelting within an area //grab the x and
					 * z coords of the mob double xCoord =
					 * EntityCobaltKatana.this.posX;
					 * System.out.println("X position is:"+xCoord); double
					 * zCoord = EntityCobaltKatana.this.posZ;
					 * System.out.println("Z position is:"+zCoord); //create
					 * limits to the radius of the effect(5x5x5 cube) int xPlus
					 * = (int)xCoord + 5; int xMinus = (int)xCoord - 5; int
					 * zPlus = (int)zCoord + 5; int zMinus = (int)zCoord - 5;
					 ...
					 * 
					 * //arbitrary flag to let the code know that the itemstack
					 * has been cooked boolean finished = false;
					 * player.inventory.setInventorySlotContents(slot,
					 * heldItem); while(player.posX > xMinus && player.posX <
					 * xPlus && player.posZ > zMinus && player.posZ < zPlus &&
					 * finished == false){
					 * System.out.println("player is within the radius");
					 * //while the cooked item stack size is still less than the
					 * held item item stack //set the slot to the stack adding
					 * the stack size if(heldItem.stackSize < stackSize
					 * )heldItem.stackSize++; finished = true; } //reduce the
					 * number of attempts by one --attempts;
					 */
	...
	/**
	 * Causes this CK to become angry at the supplied Entity (which will be a
	 * player).
	 */
	...
	public void onUpdate() {
		...
		if (MoFurnacesMod.isHalloween == true) {
			...
			float y1 = (float) this.posY + 0.0F + random.nextFloat() * 6.0F
					/ 16.0F;
			float z1 = (float) this.posZ;
			float f = 0.52F;
			float f1 = random.nextFloat() * 0.6F - 0.3F;
			// spawn in spell particles around the entity
			world.spawnParticle("spell", (double) (x1 - f), (double) y1,
					(double) (z1 + f1), 0.0D, 0.0D, 0.0D);
			world.spawnParticle("spell", (double) (x1 + f), (double) y1,
					(double) (z1 + f1), 0.0D, 0.0D, 0.0D);
			world.spawnParticle("spell", (double) (x1 + f1), (double) y1,
					(double) (z1 - f), 0.0D, 0.0D, 0.0D);
			world.spawnParticle("spell", (double) (x1 + f1), (double) y1,
					(double) (z1 + f), 0.0D, 0.0D, 0.0D);

			// if the player is within a 64 block radius of the mob then send 1
			// random message
			EntityPlayer entityPlayer = this.worldObj.getClosestPlayer(x1, y1,
					z1, 32);
					...
					if (hasChatted == false) {
						...
						if (randomInt == 0 || randomInt == 10 || randomInt == 7) {
							...
							entityPlayer
									.addChatMessage(new ChatComponentText(
											EnumChatFormatting.DARK_RED
													+ "A blood donor! Time to get my fill"));
						} else if (randomInt > 10 && randomInt < 20) {
							entityPlayer
									.addChatMessage(new ChatComponentText(
											EnumChatFormatting.DARK_GRAY
													+ "I spy with my little eye, some little player who's going to die"));
						} else if (randomInt > 20 && randomInt < 30) {
							entityPlayer.addChatMessage(new ChatComponentText(
									EnumChatFormatting.DARK_RED + "I see you "
							...
							entityPlayer
									.addChatMessage(new ChatComponentText(
											EnumChatFormatting.DARK_RED
													+ "Do you know how I made those scars?"));
							...
							entityPlayer
									.addChatMessage(new ChatComponentText(
											EnumChatFormatting.DARK_RED
													+ "I'm going to add you to my scarf, "
													+ entityPlayer
															.getDisplayName()));
							...
							entityPlayer
									.addChatMessage(new ChatComponentText(
											EnumChatFormatting.BLACK
													+ "I plucked a hair from the head of a dying baby! TAKE IT!"));
