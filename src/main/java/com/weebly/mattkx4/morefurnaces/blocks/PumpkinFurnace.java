...
import com.weebly.mattkx4.morefurnaces.particles.EntityPumpkinFlameFX;
...
public class PumpkinFurnace extends BlockContainer {
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		...
		this.iconFront = iconRegister.registerIcon(Strings.MODID
				+ ":"
				+ (this.isActive ? "PumpkinFurnace_front_active"
						: "PumpkinFurnace_front_idle"));
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, MoFurnacesMod.instance,
					MoFurnacesMod.guiIDPumpkinFurnace, world, x, y, z);
		}
		return true;
	}

	public static void updatePumpkinFurnaceState(boolean active,
		...
		if (active == true) {
			worldObj.setBlock(xCoord, yCoord, zCoord,
					MFMBlocks.PumpkinFurnaceActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord,
					MFMBlocks.PumpkinFurnaceIdle);
		}
		keepInventory = false;
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (itemstack.hasDisplayName()) {
			((TileEntityPumpkinFurnace) world.getTileEntity(x, y, z))
	...
	public void breakBlock(World world, int x, int y, int z, Block oldblock,
			int oldmetadata) {
		if (!keepInventory) {
			TileEntityPumpkinFurnace tileentity = (TileEntityPumpkinFurnace) world
					.getTileEntity(x, y, z);
		...
		if (this.isActive) {
			...
			if (direction == 4) {
				...
				Minecraft.getMinecraft().effectRenderer
						.addEffect(new EntityPumpkinFlameFX(world,
	...
	public static void spawnBat(World world, double xCoord, double yCoord,
			double zCoord) {
