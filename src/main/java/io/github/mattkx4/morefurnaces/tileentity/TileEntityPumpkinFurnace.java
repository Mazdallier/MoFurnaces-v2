...
import io.github.mattkx4.morefurnaces.blocks.PumpkinFurnace;
...
public class TileEntityPumpkinFurnace extends TileEntity implements ISidedInventory{
	...
	public String getInventoryName(){
		return this.hasCustomInventoryName() ? this.localizedName : "container.pumpkinFurnace";
	}
	...
	public void updateEntity(){
		...
		if(!this.worldObj.isRemote) {
			...
			if(flag != this.isBurning()) {
				flag1 = true;
				PumpkinFurnace.updatePumpkinFurnaceState(this.burnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
	...
	public static int getItemBurnTime(ItemStack itemstack){
		if(itemstack == null){
			...
			if(item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.air){
	            ...
	            if (block == Blocks.pumpkin) return 800;

			}	
			
				//insert item based fuels
	            if (item == Items.bone) return 200;
	            if (item == Items.rotten_flesh) return 200;
	            if (item == Items.gunpowder) return 200;
	            if (item == Items.spider_eye) return 200;
	            if (item == Items.fermented_spider_eye) return 400;
			}	            
