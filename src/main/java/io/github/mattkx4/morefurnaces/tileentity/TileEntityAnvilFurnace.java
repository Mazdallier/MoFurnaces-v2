...
public class TileEntityAnvilFurnace extends TileEntity implements
		ISidedInventory {
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				AnvilFurnace.updateAnvilFurnaceState(this.burnTime > 0,
	...
	public void smeltItem() {
		...
		if (this.canSmelt()) {
			...
			ItemStack itemstack = this.slots[0];
			// if the the item has damage less than the max damage(full
			// repaired) but greater than zero(broken)
			if (itemstack.getItemDamage() < itemstack.getMaxDamage()
				...
				itemstack.setItemDamage(currentDamage
						- FurnaceVariables.ANVIL_FURNACE_DAMAGE_FIXED);
