...
public class MFMUpdateNotifier {
	...
	// this constructor is not really needed, but I'm keeping it in case
	// deleting it has unintended consequences
	 ...
	 * EnumChatFormatting.RESET + "] "+ "Update Notifier has been disabled."));
	 * return; } else {
	 * Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new
	 * ChatComponentText("[" + EnumChatFormatting.BLUE + "MoFurnacesMod" +
	 * EnumChatFormatting.RESET + "] "+
	 * "Incorrect value in UpdateNotifierSettings.txt. UpdateNotifier cancelled."
	 * )); return; } }
	 */
	@SubscribeEvent
	...
	public void run() {
		// So the problem described is that if the player is not connected to
		// the internet, then the mod fails spectacularly
		// throwing an exception the reason is because nothing is done if the
		// buffered reader does not access the internet
		// First solution, apply a return statement if the an exception occurs.
		ArrayList<String> data = new ArrayList<String>();
		// Throw this for me
		try {
			URL accessedFile = new URL(
					"https://raw.githubusercontent.com/Mattkx4/MoFurnaces-v2/master/update/update.txt.txt");
			...
			Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(
					new ChatComponentText("[" + EnumChatFormatting.BLUE
							...
							+ "Unable to access update resource."));
			Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(
					new ChatComponentText(EnumChatFormatting.DARK_PURPLE
		...
		for (int arrayLocation = 0; arrayLocation < 3; arrayLocation++) {
			...
			if (Integer.valueOf(currentVersion[arrayLocation]) < Integer
					.valueOf(latestVersion[arrayLocation])) {
						...
						.printChatMessage(
								new ChatComponentText(
										"["
												...
												+ "] "
												+ EnumChatFormatting.DARK_PURPLE
												+ "A new update is available, Version: "
				...
				Minecraft.getMinecraft().ingameGUI.getChatGUI()
						.printChatMessage(
								new ChatComponentText("["
										...
										+ "Download here:"));
				Minecraft.getMinecraft().ingameGUI.getChatGUI()
						.printChatMessage(new ChatComponentText(downloadLink));
				// outOfDate = true;
			} else if (Integer.valueOf(currentVersion[arrayLocation]) > Integer
					.valueOf(latestVersion[arrayLocation])) {
				// Do nothing
				return;
			}
