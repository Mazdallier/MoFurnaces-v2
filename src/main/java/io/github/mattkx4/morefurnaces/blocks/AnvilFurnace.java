...
public class AnvilFurnace extends BlockContainer{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float hity, float hitz){
		if(!world.isRemote) {
			FMLNetworkHandler.openGui(player, MoFurnacesMod.instance, MoFurnacesMod.guiIDAnvilFurnace, world, x, y, z);
		}
		return true;
	}

	public static void updateAnvilFurnaceState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack){
		...
		if(itemstack.hasDisplayName()){
			((TileEntityAnvilFurnace)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
		}
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldmetadata){
        if (!keepInventory){
            TileEntityAnvilFurnace tileentity = (TileEntityAnvilFurnace)world.getTileEntity(x, y, z);
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random){
        if (this.isActive){
            ...
            Minecraft.getMinecraft().effectRenderer.addEffect(new EntityRedstoneFlameFX(world, (double)(x1 - f), (double)y1 + 0.5F, (double)(z1 + f1), 0.0D, 0.0D, 0.0D));
        	world.spawnParticle("smoke", (double)(x1 + f1), (double)y1 + 0.5F, (double)(z1 + f1), 0.0D, 0.0D, 0.0D);
        	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityRedstoneFlameFX(world, (double)(x1 + f), (double)y1 + 0.5F, (double)(z1 + f1), 0.0D, 0.0D, 0.0D));
        	world.spawnParticle("smoke", (double)(x1 + f1), (double)y1 + 0.5F, (double)(z1 - f1), 0.0D, 0.0D, 0.0D);
        	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityRedstoneFlameFX(world, (double)(x1 + f1), (double)y1 + 0.5F, (double)(z1 - f), 0.0D, 0.0D, 0.0D));
        	world.spawnParticle("smoke", (double)(x1 + f1), (double)y1 + 0.5F, (double)(z1 + f1), 0.0D, 0.0D, 0.0D);
        	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityRedstoneFlameFX(world, (double)(x1 + f1), (double)y1 + 0.5F, (double)(z1 + f), 0.0D, 0.0D, 0.0D));
        }
    }
