...
import io.github.mattkx4.morefurnaces.particles.EntityPumpkinFlameFX;
...
public class PumpkinFurnace extends BlockContainer{
	...
	public PumpkinFurnace(boolean isActive) {
		super(Material.rock);
	...
	public Item getItemDropped(int i, Random random, int j){
		return Item.getItemFromBlock(MFMBlocks.PumpkinFurnaceIdle);	
	}	
	...
	public void registerBlockIcons(IIconRegister iconRegister){
		this.blockIcon = iconRegister.registerIcon(Strings.MODID+":PumpkinFurnace_side");
		this.iconFront = iconRegister.registerIcon(Strings.MODID+":"+(this.isActive ? "PumpkinFurnace_front_active" : "PumpkinFurnace_front_idle"));
		this.iconTop = iconRegister.registerIcon(Strings.MODID+":PumpkinFurnace_top");
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float hity, float hitz){
		if(!world.isRemote) {
			FMLNetworkHandler.openGui(player, MoFurnacesMod.instance, MoFurnacesMod.guiIDPumpkinFurnace, world, x, y, z);
		}
		return true;
	}

	public static void updatePumpkinFurnaceState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		if(active == true){
			worldObj.setBlock(xCoord, yCoord, zCoord, MFMBlocks.PumpkinFurnaceActive);
		}else{
			worldObj.setBlock(xCoord, yCoord, zCoord, MFMBlocks.PumpkinFurnaceIdle);
		}
		keepInventory = false;
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return new TileEntityPumpkinFurnace();
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack){
		...
		if(itemstack.hasDisplayName()){
			((TileEntityPumpkinFurnace)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
		}
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldmetadata){
        if (!keepInventory){
            TileEntityPumpkinFurnace tileentity = (TileEntityPumpkinFurnace)world.getTileEntity(x, y, z);
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random){ 	
        ...
        if (this.isActive){
            ...
            if(direction == 4){
				...
				Minecraft.getMinecraft().effectRenderer.addEffect(new EntityPumpkinFlameFX(world, (double)(x1 - f), (double)y1, (double)(z1 + f1), 0.0D, 0.0D, 0.0D));
            }else if (direction == 5){
            	world.spawnParticle("smoke", (double)(x1 + f), (double)y1, (double)(z1 + f1), 0.0D, 0.0D, 0.0D);
            	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityPumpkinFlameFX(world, (double)(x1 + f), (double)y1, (double)(z1 + f1), 0.0D, 0.0D, 0.0D));
        	}else if (direction == 2){
            	world.spawnParticle("smoke", (double)(x1 + f1), (double)y1, (double)(z1 - f), 0.0D, 0.0D, 0.0D);
            	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityPumpkinFlameFX(world, (double)(x1 + f1), (double)y1, (double)(z1 - f), 0.0D, 0.0D, 0.0D));  
        	}else if (direction == 3){
            	world.spawnParticle("smoke", (double)(x1 + f1), (double)y1, (double)(z1 + f), 0.0D, 0.0D, 0.0D);
            	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityPumpkinFlameFX(world, (double)(x1 + f1), (double)y1, (double)(z1 + f), 0.0D, 0.0D, 0.0D));       
        	}
