...
import io.github.mattkx4.morefurnaces.particles.EntityBrickFlameFX;
...
public class BrickFurnaceT2 extends BlockContainer{
	...
	public BrickFurnaceT2(boolean isActive) {
		super(Material.rock);
	...
	public Item getItemDropped(int i, Random random, int j){
		return Item.getItemFromBlock(MFMT2Blocks.BrickFurnaceT2Idle);	
	}	
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float hity, float hitz){
		...
		if(player.getCurrentEquippedItem() != null) {
			if(player.getCurrentEquippedItem().getItem() == MFMItems.Tier3Device) {
				TileEntityBrickFurnaceT2 tileentity = (TileEntityBrickFurnaceT2)world.getTileEntity(x, y, z);
				...
				world.setBlock(x, y, z, MFMT3Blocks.BrickFurnaceT3Idle);
				world.setBlockMetadataWithNotify(x, y, z, i, 2);
				//sets whether or not the new furnace is active of inactive
				BrickFurnaceT3.updateBrickFurnaceT3State(isActive, world, x, y, z);
				
				//set the tile entity itemstacks and variable times
				TileEntityBrickFurnaceT3 tileentityT3 = (TileEntityBrickFurnaceT3)world.getTileEntity(x, y, z);
		...
		if(!world.isRemote) {
			FMLNetworkHandler.openGui(player, MoFurnacesMod.instance, MoFurnacesMod.guiIDBrickFurnaceT2, world, x, y, z);
		}
		return true;
	}

	public static void updateBrickFurnaceT2State(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		if(active == true){
			worldObj.setBlock(xCoord, yCoord, zCoord, MFMT2Blocks.BrickFurnaceT2Active);
		}else{
			worldObj.setBlock(xCoord, yCoord, zCoord, MFMT2Blocks.BrickFurnaceT2Idle);
		}
		keepInventory = false;
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return new TileEntityBrickFurnaceT2();
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack){
		...
		if(itemstack.hasDisplayName()){
			((TileEntityBrickFurnaceT2)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
		}
	}
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random){ 	
        ...
        if (this.isActive){
            ...
            if(direction == 4){
				...
				Minecraft.getMinecraft().effectRenderer.addEffect(new EntityBrickFlameFX(world, (double)(x1 - f), (double)y1, (double)(z1 + f1), 0.0D, 0.0D, 0.0D));
            	...
            	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityBrickFlameFX(world, (double)(x1 + f1), (double)y1, (double)(z1 - f), 0.0D, 0.0D, 0.0D));  
        	}else if (direction == 3){
            	world.spawnParticle("smoke", (double)(x1 + f1), (double)y1, (double)(z1 + f), 0.0D, 0.0D, 0.0D);
            	Minecraft.getMinecraft().effectRenderer.addEffect(new EntityBrickFlameFX(world, (double)(x1 + f1), (double)y1, (double)(z1 + f), 0.0D, 0.0D, 0.0D));       
        	}
