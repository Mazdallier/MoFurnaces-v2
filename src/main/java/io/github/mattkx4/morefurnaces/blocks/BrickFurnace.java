...
public class BrickFurnace extends BlockContainer{
		...
		public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float hity, float hitz){
			...
			if(player.getCurrentEquippedItem() != null) {
				if(player.getCurrentEquippedItem().getItem() == MFMItems.Tier2Device) {
					...
					BrickFurnaceT2.updateBrickFurnaceT2State(isActive, world, x, y, z);
	...
	public static void updateBrickFurnaceState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		if(active == true){
			worldObj.setBlock(xCoord, yCoord, zCoord, MFMBlocks.BrickFurnaceActive);
		}else{
			worldObj.setBlock(xCoord, yCoord, zCoord, MFMBlocks.BrickFurnaceIdle);
		}
		keepInventory = false;
