...
public class BrickFurnace extends BlockContainer {
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		...
		this.iconFront = iconRegister.registerIcon(Strings.MODID
				+ ":"
				+ (this.isActive ? "BrickFurnace_front_active"
						: "BrickFurnace_front_idle"));
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, MoFurnacesMod.instance,
					MoFurnacesMod.guiIDBrickFurnace, world, x, y, z);
		}
		return true;
	}

	public static void updateBrickFurnaceState(boolean active, World worldObj,
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (itemstack.hasDisplayName()) {
			((TileEntityBrickFurnace) world.getTileEntity(x, y, z))
	...
	public static void upgradeActive(World worldObj, int xCoord, int yCoord,
			int zCoord, String upgrade, boolean isActive) {
		switch (upgrade) {
		case "brightness":
			// get the closes entitymob (15 blocks away)
			// get the boundary for the effect
			AxisAlignedBB boundary = AxisAlignedBB.getBoundingBox(xCoord - 8,
					yCoord - 8, zCoord - 8, xCoord + 8, yCoord + 8, zCoord + 8);
			// create a list of entities with all the EntityMob's within the
			// boundary
