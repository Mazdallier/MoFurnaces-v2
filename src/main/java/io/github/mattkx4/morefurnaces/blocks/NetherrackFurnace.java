...
public class NetherrackFurnace extends BlockContainer {
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, MoFurnacesMod.instance,
					MoFurnacesMod.guiIDNetherrackFurnace, world, x, y, z);
		}
		return true;
	}

	/*
	 * public static void updateNetherrackFurnaceState(boolean active, World
	 * worldObj, int xCoord, int yCoord, int zCoord) { int i =
	 * worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
	 * 
	 * TileEntity tileentity = worldObj.getTileEntity(xCoord, yCoord, zCoord);
	 * keepInventory = true;
	 * 
	 * if(active == true){ worldObj.setBlock(xCoord, yCoord, zCoord,
	 * MFMBlocks.NetherrackFurnaceActive); }else{ worldObj.setBlock(xCoord,
	 * yCoord, zCoord, MFMBlocks.NetherrackFurnaceIdle); } keepInventory =
	 * false;
	 * 
	 * worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, i, 2);
	 * 
	 * if(tileentity != null){ tileentity.validate();
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (itemstack.hasDisplayName()) {
			((TileEntityNetherrackFurnace) world.getTileEntity(x, y, z))
	...
	public void breakBlock(World world, int x, int y, int z, Block oldblock,
			int oldmetadata) {
		if (!keepInventory) {
			TileEntityNetherrackFurnace tileentity = (TileEntityNetherrackFurnace) world
					.getTileEntity(x, y, z);
		...
		if (direction == 4) {
			...
			Minecraft.getMinecraft().effectRenderer
					.addEffect(new EntityRedstoneFlameFX(world,
							(double) (x1 - f), (double) y1, (double) (z1 + f1),
							0.0D, 0.0D, 0.0D));
			...
			Minecraft.getMinecraft().effectRenderer
					.addEffect(new EntityRedstoneFlameFX(world,
							(double) (x1 + f1), (double) y1, (double) (z1 - f),
							0.0D, 0.0D, 0.0D));
		} else if (direction == 3) {
			world.spawnParticle("smoke", (double) (x1 + f1), (double) y1,
					(double) (z1 + f), 0.0D, 0.0D, 0.0D);
