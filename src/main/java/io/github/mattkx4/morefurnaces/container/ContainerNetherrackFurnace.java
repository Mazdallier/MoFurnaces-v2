...
public class ContainerNetherrackFurnace extends Container {
	 ...
	 */
	public ContainerNetherrackFurnace(InventoryPlayer inventory,
			TileEntityNetherrackFurnace tileentity) {
	...
	public void addCraftingToCrafters(ICrafting icrafting) {
		...
		icrafting.sendProgressBarUpdate(this, 0,
				this.netherrackFurnace.cookTime);
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			// this here deals shift clicking the output slot which has been
			// ID'd as slot 1
			// thus this needs to be par2 == 1
			// mergeItemStack needs to go from slot 2(the first inventory slot)
			// to slot 38(the last inventory slot)
			if (par2 == 1) {
			...
			// this should only be for inputs, as there is only one input (0)
			// this needs to say
			// par2 != 0
			else if (par2 != 0) {
				...
				// removal of the condition for if the itemstack is a fuel (as
				// no fuel is needed)
				/*
				 * else if (TileEntityNetherrackFurnace.isItemFuel(itemstack1))
				 * { if (!this.mergeItemStack(itemstack1, 1, 2, false)) { return
				 * null; } }
				 */
				// otherwise if the shift clicked slot is any of the inventory
				// slots (2 to 29 not 3 to 30)
				// changed all values down by one to properly reflect the number
				// of slots
